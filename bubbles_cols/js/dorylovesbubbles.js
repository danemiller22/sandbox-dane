...
//function beginMapping(data) {
    ...
    // Create the canvas
    var canvasSize = 2000; //window.innerWidth; //900;
	...
	function generateSelect(inputID, backend_list, frontend_list) {
		...
		//select.value = "EISP";
		//select.value = "EmailLists";
		//select.value = "MailJet (ODBC)";
		select.value = "UiOpener (ODBC)";
		
		
		// Add recursion options
		var selectRecsv = document.getElementById("selectRecsv");
		//while (selectRecsv.length > 0) selectRecsv.remove(0); // Remove options
		for(var i=1;i<=5;i++) {
			var option = document.createElement("option");
			option.text = i;
			option.value = i;
			selectRecsv.appendChild(option);
		}
		document.getElementById("selectRecsv").value = 1;
	}
	
	function generateMapSelect(dbname) {
	...
	var myfiletxt = "";
	var myCSV = [];
	//readTextFile("qryExport.txt", fileLoaded);
				...
				for(var j=0; j<collisionchecklist.length; j++) {
					if (collisionchecklist[i] !== collisionchecklist[j]) 
					{
						...
						if (bubble1.hasOverlap(bubble2) === true)
						{
							...
							// X
							//if (bubble1.y >= bubble2.y) bubble1.y += (bubble1.getHeight() + bubble2.getHeight() + 15);
							//else bubble1.y -= (bubble1.getHeight() + bubble2.getHeight() + 15);
							bubble1.y += (bubble1.getHeight() + bubble2.getHeight() + 15);
							// Y
							if (bubble1.hasOverlap(bubble2) === true)
							{
								//if (bubble1.x >= bubble2.x) bubble1.x += (bubble1.getWidth() + bubble2.getWidth() + 15);
								//else bubble1.x -= (bubble1.getWidth() + bubble2.getWidth() + 15);
								bubble1.x += (bubble1.getWidth() + bubble2.getWidth() + 15);
							}
