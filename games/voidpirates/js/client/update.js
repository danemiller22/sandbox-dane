...
function renderMinimap(ctx, canvas, offsetScale, minimap)
{
	...
	for(var i=0; i<minimap.map.length; i++) {
		if (minimap.map[i].actortype == "sun") {
			...
			ctx.strokeStyle = 'white';
			ctx.lineWidth = 0.4;
			ctx.arc(xOffset, yOffset, (1800*offsetScale), 0, 2 * Math.PI);
			ctx.stroke();
...
function rendeMinimapEntity(ctx, canvas, offsetScale, entity)
{
	...
	/*
	//if (entity.actortype == "sun") ctx.rect(xOffset-(1548*offsetScale*0.5), yOffset-(1548*offsetScale*0.5), (1548*offsetScale), (1548*offsetScale));
	if (entity.actortype && entity.actortype == "sun") ctx.arc(xOffset, yOffset, (1548*offsetScale*0.5), 0, 2 * Math.PI); 	
	else ctx.rect(xOffset-1, yOffset-1, 3, 3);
	*/
	
	if (entity.actortype && entity.actortype == "sun") ctx.arc(xOffset, yOffset, (1548*offsetScale*0.5), 0, 2 * Math.PI);
	else if (entity.color == "green") ctx.arc(xOffset, yOffset, (200*offsetScale*0.5), 0, 2 * Math.PI);
	else ctx.rect(xOffset-1, yOffset-1, 3, 3);
